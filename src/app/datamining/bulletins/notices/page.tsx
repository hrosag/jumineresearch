...
export default function NoticesPage() {
  ...
  // densidade de pontos para decidir amostragem
  const pointBudget = Math.max(300, Math.floor(chartWidth * 0.8)) // limite visual
  const shouldSample = filtered.length > pointBudget

  const chartData: ChartPoint[] = useMemo(() => {
    if (!filtered.length) return []
    // sem amostragem quando poucos pontos -> preserva visual bonito
    if (!shouldSample) {
  ...
  // ticks adaptativos: mensal para janelas longas, semanal para curtas
  const xTicks = useMemo(() => {
    ...
    const stepMode = totalDays <= 120 ? 'week' : 'month'
    const cur = new Date(Date.UTC(d0.getUTCFullYear(), d0.getUTCMonth(), stepMode === 'week' ? d0.getUTCDate() - (d0.getUTCDay() || 7) + 1 : 1))
    while (cur <= d1) {
      out.push(cur.getTime())
      if (stepMode === 'week') cur.setUTCDate(cur.getUTCDate() + 7)
  ...
  const resetFilters = () => {
    setSelectedCompanies([]); setSelectedTypes([])
    setStartDate(globalMin); setEndDate(globalMax)
    setShowEvents(50)
  }

  if (loading) return <p className="p-4">Carregando…</p>

  // altura mais compacta quando 1 empresa para evitar “vazio”
  const chartHeight = effectiveCompanies.length <= 1 ? 220 : 400
      ...
      <div ref={chartWrapRef}>
        <ResponsiveContainer width="100%" height={chartHeight}>
          <ScatterChart margin={{ top: 10, right: 20, bottom: 10, left: 10 }}>
            ...
            />
            {/* quando 1 empresa, esconda e comprima o eixo */}
            <YAxis type="category" dataKey="company" tick={effectiveCompanies.length > 1} axisLine={false} />
            ...
            {/* proíbe linhas entre pontos e usa círculos pequenos */}
            <Scatter data={chartData} isAnimationActive={false} line={false} shape={<circle r={2} />} />
            {/* zoom horizontal */}
