...
type RowMixed = Row & { _mixed?: boolean };
...
type Bucket = { pure: RowMixed[]; mixed: RowMixed[] };
...
function bucketize(rows: Row[]) {
  ...
  for (const r of rows) {
    ...
    const r2: RowMixed = { ...r, _mixed: meta.mixed };
...
function valueForSort(r: RowMixed, key: SortKey): string {
  switch (key) {
    case "company":
      return r.company ?? "";
    case "ticker_root":
      return r.ticker_root ?? "";
    case "composite_key":
...
function sortRows(rows: RowMixed[], key: SortKey, dir: SortDir) {
...
function useTextFilter(rows: RowMixed[]) {
